...
const Hero = () => {
  ...
  return (
    <section ref={sectionRef} className="relative min-h-screen flex items-end pb-24 pt-28 overflow-hidden bg-[#082f49]">
      ...
      <div className="absolute left-0 top-1/4 bottom-1/4 w-[3px] bg-gradient-to-b from-transparent via-sky-400 to-transparent z-10 hidden lg:block"></div>
        ...
        <div className="max-w-5xl">
          ...
          <motion.div variants={staggerContainer} initial="hidden" animate="visible">
            ...
            <motion.div variants={fadeUp} className="flex items-center gap-4 mb-8">
              ...
              <span className="text-sky-400 font-bold uppercase tracking-[0.3em] text-xs">South India's #1 Travel Agency</span>
            </motion.div>
            ...
            <motion.h1 variants={fadeUp} className="text-6xl sm:text-7xl md:text-8xl xl:text-9xl font-black text-white leading-[0.95] tracking-tight mb-10">
              ...
              <span className="relative inline-block">
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 via-yellow-300 to-sky-500">
                  Awaits You
                </span>
              </span>
            </motion.h1>
            ...
            <motion.div variants={fadeUp} className="flex flex-col sm:flex-row items-start sm:items-center gap-6 mb-16">
              <Link
                to="/packages"
                className="group inline-flex items-center gap-3 bg-sky-400 hover:bg-sky-300 text-[#060d1a] font-black text-lg px-10 py-4 rounded-full transition-all duration-300 shadow-[0_0_40px_rgba(251,191,36,0.4)] hover:shadow-[0_0_60px_rgba(251,191,36,0.6)]"
              >
                Explore Packages
              ...
              <Link
                to="/contact"
                className="inline-flex items-center gap-3 text-white/80 hover:text-white font-bold border-b border-white/30 hover:border-sky-400 hover:text-sky-400 transition-all pb-0.5"
              >
                Plan My Trip →
              </Link>
            </motion.div>
            ...
            <motion.div variants={staggerContainer} className="flex flex-wrap gap-6">
              {[
                { icon: <Award className="w-5 h-5 text-sky-400" />, label: '21+ Years of Excellence' },
                { icon: <Shield className="w-5 h-5 text-sky-400" />, label: '100% Safe & Trusted' },
                { icon: <Compass className="w-5 h-5 text-sky-400" />, label: '6 Continents Covered' },
              ].map((b, i) => (
