...
const Testimonials = () => {
  const reviews = [
    ...
    },
    {
      ...
      bg: "from-sky-500/20 to-yellow-500/10",
    }
  ];
  ...
  return (
    <section className="py-28 bg-[#082f49] relative overflow-hidden">
      {/* Subtle radial glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[400px] bg-sky-400/5 rounded-full blur-[120px] pointer-events-none"></div>
      ...
      <div className="container mx-auto px-6 lg:px-16 relative z-10">
        ...
        >
          ...
          <div>
            <p className="text-sky-400 font-bold uppercase tracking-[0.25em] text-xs mb-2">Client Stories</p>
        ...
        >
          {reviews.map((r) => (
            <motion.div
              ...
              className={`relative rounded-2xl p-8 border border-white/8 bg-gradient-to-br ${r.bg} backdrop-blur-sm overflow-hidden group hover:border-sky-400/30 transition-colors`}
            >
              <Quote className="absolute top-5 right-5 w-12 h-12 text-sky-400/10" />
              ...
              <div className="flex items-center gap-4 border-t border-white/8 pt-6">
                <div className="w-12 h-12 rounded-xl bg-sky-400/20 border border-sky-400/30 flex items-center justify-center font-black text-sky-300 text-sm flex-shrink-0">
                  {r.avatar}
                </div>
                <div>
                  <h4 className="font-black text-white text-sm">{r.name}</h4>
                  <p className="text-sky-400/70 text-xs font-semibold mt-0.5">{r.type}</p>
                </div>
              </div>
            </motion.div>
          ))}
        ...
        >
          ...
          <a
            href="/contact"
            className="inline-flex items-center gap-3 bg-sky-400 hover:bg-sky-300 text-[#082f49] font-black text-base px-10 py-4 rounded-full transition-all shadow-[0_0_40px_rgba(251,191,36,0.3)] hover:shadow-[0_0_60px_rgba(251,191,36,0.5)]"
          >
            Plan My Trip Now →
          </a>
        </motion.div>
